...
const findAndUpdateAgeByName = (req, res) => {
  if (!req.body.name) {
  ...
  return Dog.findByName(req.body.name, (err, doc) => {
    ...
    if (!doc) {
      return res.json({ error: `Could not find a dog with name ${req.body.name}` });
    }
    ...
    savePromise.catch(promiseErr => res.json({ promiseErr }));
